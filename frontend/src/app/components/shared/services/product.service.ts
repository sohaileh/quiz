...
*/
  ...
  public addToCompare(product: Product): Product | boolean {
    ...
    if (this.hasProduct(product)) {
      ...
      this.snackBar.open('The product  ' + product.catalogItemX + ' already added to comparison list.', '×', { panelClass: 'error', verticalPosition: 'top', duration: 3000 });

    } else {
      if (products.length < 4)
        products.push(product);
      message = 'The product ' + product.catalogItemX + ' has been added to comparison list.';
      status = 'success';
