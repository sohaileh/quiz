...
export class WishlistService {
  ...
  public hasProduct(product: Product): boolean {
    const item = products.find(item => item.catalogItem == product.id);
  ...
  public addToWishlist(product: Product): Product | boolean {
    ...
    if (this.hasProduct(product)) {
      item = products.filter(item => item.catalogItem == product.id)[0];
      const index = products.indexOf(item);
    } else {
      products.push(product);
    }
    message = 'The product ' + product.catalogItemX + ' has been added to wishlist.';
    status = 'success';
