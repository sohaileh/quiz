   ...
   public addToCart(product: Product, quantity: number) {
    ...
    let message, status;
     ...
     let hasItem = products.find((items, index) => {
       if(items.product.catalogItemX == product.catalogItemX) {
         let qty = products[index].quantity + quantity;
         let stock = this.calculateStockCounts(products[index], quantity);
         if(qty != 0 && stock) {
           products[index]['quantity'] = qty;
           message = 'The product ' + product.catalogItemX + ' has been added to cart.';
           status = 'success';
...
// public clearALL(item: CartItem) {
//   if (item === undefined) return false;
//     const index = products.indexOf(item);
//     products.splice(0, product.length-);
//     localStorage.setItem("cartItem", JSON.stringify(products));
// }
